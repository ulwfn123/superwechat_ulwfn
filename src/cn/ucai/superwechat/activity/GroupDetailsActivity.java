...
//        addGroupMembers(groupId,n);
	...
	protected void onDestroy() {
		super.onDestroy();
		instance = null;
        if (mReceiver != null) {  // 群组添加人员的广播监听销毁
...
//				holder.textView.setText(username);  //   显示 群组中所有的 成员的 昵称和头像时注释
//				UserUtils.setUserAvatar(getContext(), username, holder.imageView);

                UserUtils.setAppMemberNick(groupId, username, holder.textView); //显示 群组中所有的 成员的 昵称和头像
    ...
    //  为 群组 添加人员  所新建的内部类 继承了    广播监听
    class UndateMemberReceiver extends BroadcastReceiver {
