...
//					unreadAddressLable.setText(String.valueOf(count));
	...
	public class MyContactListener implements EMContactListener {
								...
								if (result != null && result.isRetMsg()) {
									...
									if (user != null) {
										if (!FuliCenterApplication.getInstance().getUserMap().containsKey(user.getMUserName())) {
                                            ...
                                            FuliCenterApplication.getInstance().getUserlist().add(user);  // 全局变量添加好友
		...
		public void onContactDeleted(final List<String> usernameList) {
			...
			// 被删除
			// 删除好友
			final String currentUserName = FuliCenterApplication.getInstance().getUserName();
						...
						.execute(new OkHttpUtils2.OnCompleteListener<Result>() {
							...
							public void onSuccess(Result result) {
								if (result.isRetMsg()) {
									...
									UserAvatar u = FuliCenterApplication.getInstance().getUserMap().remove(username);
...
//			for (String username : usernameList) {  删除联系人时被注释
